...
import { FragmentationBySize as FragmentationBySizeData } from '../../types';
import { Card } from '../common/Card';
import { LoadingOverlay } from '../common/LoadingSpinner';
import { EmptyState } from '../common/StateIndicators';

interface FragmentationBySizeProps {
  data: FragmentationBySizeData[];
  height?: number;
  isLoading?: boolean;
}
...
const CORAL_TYPE_COLORS = {
  Natural: '#d62828',  // Red/coral for shrinkage/fragmentation
  Restored: '#f77f00', // Orange for restored
};

function FragmentationBySizeComponent({
  data,
  height = 400,
  isLoading = false,
}: FragmentationBySizeProps) {
  const safeData = useMemo(() => {
    ...
    return data.filter(
      (d) =>
        ...
        typeof d.pct_shrinking === 'number' &&
        !isNaN(d.pct_shrinking) &&
  ...
  useEffect(() => {
    ...
    const yMax = Math.max(50, d3.max(safeData, (d) => d.ci_upper) || 40);
    ...
    sizeGroups.each(function (sizeClass) {
      ...
      group
        ...
        .attr('y', (d) => yScale(d.pct_shrinking))
        .attr('width', x1Scale.bandwidth())
        .attr('height', (d) => innerHeight - yScale(d.pct_shrinking))
      ...
      group
        ...
        .attr('y1', (d) => yScale(Math.min(100, d.ci_upper)))
      ...
      group
        ...
        .attr('y1', (d) => yScale(Math.min(100, d.ci_upper)))
        .attr('y2', (d) => yScale(Math.min(100, d.ci_upper)))
        .attr('stroke', '#374151')
        .attr('stroke-width', 1.5);
      ...
      group
        ...
        .attr('y', (d) => yScale(d.pct_shrinking) - 8)
        .attr('text-anchor', 'middle')
        .attr('font-size', '10px')
        .attr('font-family', 'JetBrains Mono, monospace')
        .attr('fill', '#1a1a2e')
        .text((d) => `${d.pct_shrinking.toFixed(0)}%`);
    ...
    g.append('text')
      ...
      .attr('font-size', '12px')
      .attr('fill', '#5d6d7e')
      .text('Partial Mortality / Shrinkage (%)');
    ...
    svg
      ...
      .attr('fill', '#0a3d62')
      .text('Partial Mortality by Size: Natural vs Restored');
    ...
    svg
      ...
      .text('Higher values indicate greater partial mortality (tissue loss/fragmentation)');
    ...
    return (
      <Card className="relative overflow-hidden">
        <EmptyState
          ...
          description="No fragmentation data available for comparison."
          size="md"
        />
      </Card>
    );
...
export const FragmentationBySize = memo(FragmentationBySizeComponent);
